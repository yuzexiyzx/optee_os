...
static enum hisi_drv_status hpre_rsa_parse_sqe(void *bd, void *info __unused)
{
	...
	if (done != HPRE_HW_TASK_DONE || err) {
		EMSG("HPRE do rsa fail! done=0x%"PRIX16", etype=0x%"PRIX16, done, err);
		return HISI_QM_DRVCRYPT_IN_EPARA;
	}
...
static int32_t hpre_rsa_gen_p_q(size_t nbits, struct rsa_keypair *key)
{
	...
	while (true) {
		MBEDTLS_MPI_CHK(mbedtls_mpi_gen_prime(&rsa.P, nbits >> 1,
				prime_quality, hpre_rsa_get_random_data, NULL));
...
alloc_fail:
	EMSG("HPRE rsa alloc keypair fail");
...
static size_t hpre_rsa_get_hw_kbytes(size_t key_bits)
{
	...
	if (key_bits <= 1024)
		...
		EMSG("Invalid key_bits[%"PRIu32"]", key_bits);
...
static void hpre_rsa_params_free(struct hpre_rsa_msg *msg)
{
	...
	case HPRE_ALG_NC_NCRT:
		if (msg->is_private) {
			memzero_explicit(msg->prikey, msg->key_bytes << 1);
			free(msg->prikey);
		} else
			free(msg->pubkey);
		break;
	case HPRE_ALG_NC_CRT:
		if (msg->is_private) {
			...
			memzero_explicit(msg->prikey, len);
			free(msg->prikey);
		}
		break;
	default:
						    ...
						    struct rsa_keypair *keypair)
{
	...
	crypto_bignum_bn2bin(keypair->e, e);
	crypto_bignum_bn2bin(keypair->p, p);
	crypto_bignum_bn2bin(keypair->q, q);
	e_len = crypto_bignum_num_bytes(keypair->e);
	p_len = crypto_bignum_num_bytes(keypair->p);
	q_len = crypto_bignum_num_bytes(keypair->q);
					...
					size_t key_bits)
{
	...
	ret = hpre_rsa_keypair_bn2bin(msg, keypair);
	if (ret) {
...
static TEE_Result hpre_rsa_gen_keypair(struct rsa_keypair *key, size_t key_bits)
{
	...
	if (!key || !key->n || !key->e || !key->d || !key->p || !key->q ||
		...
		EMSG("Invalid rsa gen keypair input parameter");
		return TEE_ERROR_BAD_PARAMETERS;
	}
...
static enum hisi_drv_status hpre_rsa_encrypt_alloc(struct hpre_rsa_msg *msg)
{
	...
	if (!msg->pubkey_dma) {
		EMSG("Fail to get pubkey dma addr");
		free(data);
		return HISI_QM_DRVCRYPT_EFAULT;
	}
...
static enum hisi_drv_status hpre_rsa_encrypt_bn2bin(struct hpre_rsa_msg *msg,
						    struct drvcrypt_rsa_ed *rsa_data)
{
	struct rsa_public_key *key = (struct rsa_public_key *)rsa_data->key.key;
	uint32_t e_len = 0;
	uint32_t n_len = 0;
	enum hisi_drv_status ret = HISI_QM_DRVCRYPT_NO_ERR;
	uint8_t *n = NULL;
	...
	ret = hpre_bin_from_crypto_bin(msg->pubkey, msg->pubkey, msg->key_bytes,
				     e_len);
	if (ret) {
	...
	ret = hpre_bin_from_crypto_bin(msg->in, rsa_data->message.data,
				      msg->key_bytes, rsa_data->message.length);
	if (ret)
...
static TEE_Result rsa_nopad_encrypt(struct drvcrypt_rsa_ed *rsa_data)
{
	...
	if (rsa_data->message.length > n_bytes) {
		EMSG("Invalid msg length[%"PRIu32"]", rsa_data->message.length);
		return TEE_ERROR_BAD_PARAMETERS;
	}
					 ...
					 uint8_t *out)
{
	...
	/* PKCS_V1.5 foramt 0x00 || 0x02 || PS non-zero || 0x00 || M */
...
static TEE_Result rsa_pkcs_encrypt(struct drvcrypt_rsa_ed *rsa_data)
{
	...
	rsa_enc_info.message.data = (uint8_t *)malloc(n_bytes);
...
hpre_rsa_crt_decrypt_bn2bin(struct hpre_rsa_msg *msg,
					   struct drvcrypt_rsa_ed *rsa_data)
{
	struct rsa_keypair *key = (struct rsa_keypair *)rsa_data->key.key;
...
static TEE_Result rsa_nopad_decrypt(struct drvcrypt_rsa_ed *rsa_data)
{
	...
	if (rsa_data->cipher.length > n_bytes) {
		EMSG("Invalid cipher length[%"PRIu32"]", rsa_data->cipher.length);
		return TEE_ERROR_BAD_PARAMETERS;
	}
...
{
	uint32_t n_bytes = rsa_data->key.n_size;
	struct drvcrypt_rsa_ed rsa_dec_info = { };
	TEE_Result ret = TEE_SUCCESS;

	memcpy(&rsa_dec_info, rsa_data, sizeof(rsa_dec_info));
	/* Alloc pkcs_v1.5 encode message data buf */
	rsa_dec_info.message.data = (uint8_t *)malloc(n_bytes);
				   ...
				   uint8_t *db, uint8_t *out, size_t *out_len)
{
	...
	if (msg_len > rsa_data->message.length) {
		EMSG("Message space is not enougth");
